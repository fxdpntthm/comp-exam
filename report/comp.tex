...
module. In Haskell, generative types are defined by hiding the data constructors
from the module exports. Consider an HTML module defined in
...
of course by using the specific functions exposed by the module.
Inhibiting manipulations and views of the data
representation is useful for avoiding leaking information in
...
A smart implementation should be able to remove the extra runtime overhead
of converting !String! to !Html! and vice versa.
...
typechecking, desugaring---are together called
the front end of the compiler. Haskell is statically typed,
which has practical and theoretical implications;
in practice, only the terms which can be verified to be type correct by
the typechecker is compiled to the core intermediate language, and in theory,
terms without types have no meanings. The prior iterations of GHC
...
the previous section and motivate why \SF with extensions is
...
general, or pricipal, type of the term. Implicit parametric~\cite{reynolds_user-defined_1978}, and adhoc
polymorphism~\cite{hall_type_1994} in Haskell can be
translated into \SF~\cite{girard_proofs_1989,reynolds_towards_1974}.
The Hindley-Milner type system can be viewed as a restricted
version of \SF. All the types in Hindley-Milner type system
are of the prenex form: the universally quantified type variables appear
at the outermost level and there are no existentially quantified type variables.
The functions from the surface level language are elaborated
to have explicit type arguments. The function's use site  in the elaborated
form needs to have all the type argument applied before the value arguments.
...
that the types, which need to be explicitly applied in the elaborated language,
needs to be computed by a procedure. Further, there needs to be
a unique value type to fill into the placeholder arguments in the elaborated term.
The burden of finding or inferring the types naturally falls on the typechecker
The uniqueness criteria for inferred type is important as the meaning
of the term is inherently tied to the type argument it is applied to and
we do not want the compiler to make arbitrary decisions for the programmer.

Although typeclasses and algebraic data types can be encoded in \SF,
GADTs, generative abstract types, and associated types are cumbersome,
if not impossible, to express using orthogonal extensions of pure \SF.
Further, the interaction of each of these seemingly different features together would
...
The key insight is that each language feature
is, encoding a notion of type equality
...
types (\pref{sec:fc-encodes-assoctypes}) are encoded in \SFC
using \emph{coercions} and \emph{coercion
  axioms} which embody the notion of type equality.
We also describe a unique feature of the system,
open type functions (\pref{sec:fc-encodes-opentypefun}) which expose
the type equality functionality to the user as a direct
...
disallowing certain types such as $\TLam a {T\App a\App Int}$. This is
...
The novel construct, coercions, $\shl{\Co}$ are proofs of type equality.
They are classified by kind level type equality predicate,
...
abstracted over by using the special coercion syntax infrastructure.
...
equality reasoning on polytypes.
In principle, \emph{coercions are types}. They are shown separately
only for the sake of presentation in \pref{fig:sfc-syntax}.
and we will use $\phi$ when the distinction is immaterial. Coercions can also
...
The novel term construct of the system is the type cast, $\shl{\Cast \Tm \Co}$.
...
expects a term of type $\sigma$, we can freely use the term $\Cast M \Co$.
The type coercions in the system can be thought of as axiomatic type equality,
similar to axiomatizing functional extensionality.
However, the type soundness argument of the system,
does not rely on any specific semantic notion of type equality.
...
do not appear in the return type. And are disjoint from the type variables $\alpha$,
$\many\TyVar \cap \many\beta = \emptyset$.
...
provide static semantics in the form judgments rules in \pref{fig:sfc-typing-co}.
...
$\trule{\I\forall}$, and $\trule{\E\forall}$. The rule
$\trule{cast}$ transforms a term $\Tm$ of type $\tau$ to a term
of type $\sigma$ using a witness coercion $\Co\co\tau\sim\sigma$.
...
the existentially bounded variables. However, the existential variables
should not escape their scope. This invariant
needs to be enforced by the elaboration step from surface syntax into
the core language as \SFC is a general calculus.
Consider the data constructor !IsEquals! and its type
...
We now analyze the runtime or the operational semantics,
of the terms of the language. The value terms fall into two
...
presentation uses call by need semantics. The terms involving
coercions can step in one of the four interesting
...
coercion computation ensures that the types are
...
\subsection{Type Computation}\label{sec:fc-encodes-assoctypes}
...
for generic programming by encoding type application~\cite{magalhaes_generic_2010}:
...
The proof of the theorems need to account for kind substitutions along with term substitutions,
but the proofs are otherwise routine tedious induction on the small step operational semantics.
