...
ways given by the rules \trule{push}, \trule{ty-push}, \trule{co-push},
or \trule{h-push} shown in the \pref{fig:op-sem-sfc}. In the rule
\trule{push}, the coercion $\Co$, which applied to the lambda term, is
...
rule \trule{ty-push} for type application
moves the coercion inside a type abstraction instantiated at the type
variable. The rule \trule{co-push} is just like \trule{push} but for
...
The most complex rule is \trule{h-push}, which makes more sense with an
