...
language C, or the use of template programming, as in the language C++.
...
types\cite{jones_qualified_1994}: the term !False + True! is
meaningless, as it does not have a well defined semantics precisely
because Booleans cannot be added together. The addition function
(!+!), thus, has the type !Num t => t -> t -> t!. This means that the
...
original Algorithm M or W, the type constraints are propagated from
the leaves of the AST\cite{lee_proofs_1998}. Even if it did, the type annotation
...
\subsection{A Quick Note About Notations}\label{sec:notations}
...
which are not coercions, are listed in \pref{fig:sfc-typing-ty}.
...
to match and make progress using the \trule{case}.  Coercion lifting plays an important
role: it ensures that the term sub-components $\Cast M_i
{\Subst}\rho_i$ are of the appropriate type. Finally, the rule \trule{co-trans}
...
and they cannot be curried. To disambiguate them from
generic type functions, the GHC convention is to call them !type family!.
...
% We skip the formalization for encoding functional dependencies until further \pref{sec:conclusion}; while there is an encoding mechanism that is sound, a complete encoding that covers all cases is an open problem.
