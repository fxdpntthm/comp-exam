...
functional programming language. It is organized as follows: We set
the scope of the research interest space in
\pref{sec:introduction}, and we then go through some quintessential
along with some advanced language features of a statically typed
functional programming language. Some of these features are specific to the
Haskell language. This serves as a \emph{programmers view} of the language.
The \pref{sec:sfc} focuses on the formalization aspects of \SFC.
This portion of the report is the \emph{compiler writers} view of the
language. \SFC in essence is \SF enriched with first class explicit
type equalities. It forms the basis of the core intermediate language
of GHC, the de facto Haskell language compiler. Adding type equalities
as a core language construct has extensive repercussions.
In the \pref{sec:sfc-encoding-features}, we sketch how type equalities
...
making the type (and kind) level computation even more expressive.
To conclude, in \pref{sec:related-work} we point out some important related
work not covered in this report and give some directions to future
work in \pref{sec:future-work}.
\end{abstract}

\maketitle
\section{Introduction}\label{sec:introduction}
Constructive mathematics and computer programming are acts of doing the same thing:
identifying and exploiting the right abstractions to solve a problem. Mathematicians
tackle it from top to bottom: by building the abstractions first and then enriching them,
to make them represent real world problems, while programming turns the process on to its
head, the programmer first writes a program, possibly incorrect, to solve a very specific
task, and then by means of iterative code refactoring, build the right level of
abstraction, while possibly fixing the incorrect program behaviors.
...
A compiler, in essence, is a program which transforms a higher level language, suitable for
humans, to a lower level language, suitable for computers. It may
preform this transformation possibly via a multi-step
process\cite{essentials_siek_2023}.
Every useful software program needs to evolve over time by means of
extensions, and a new language feature is implemented as a new
compiler feature extension. There are two ways of realizing this
extension: encode the new language feature means of existing high
level language features, or by enriching the intermediate language. In the first encoding
scheme, we just define a high level language to a high level language syntax
transformer. Some exotic language features may not be able to leverage such encoding
scheme for practical or theoretical reasons and for such cases, we enrich the intermediate
language which reflects the extensions made to the higher level language. However, in the
second case, multiple extensions to the intermediate language causes a feature clutter,
making it difficult to maintain. We then need to find the right abstraction to refactor
the intermediate language so that all different high level language features can be
encoded precisely into a unified intermediate language with fewer extensions.

In compilers for a statically typed programming languages, the type checking component, or
simply, the typechecker, can be thought of as the gate keeper. Its purpose is to ensure
that it does not let ``bad'' programs go through while flagging
appropriate offending parts of programs to the programmer.
This gate keeping is desirable because it liberates the programmer as
...
can concentrate on the other more important aspects of the program.
On the other hand, the typechecker also needs to allow the ``good'' programs
which the programmers wishes write, pass without an intense struggle.
The safety net casted by the typechecker should not become the reason
of its undesirability. The structure of types needs to be
sufficiently enriched to express complex programmer ideas, while rejecting runtime unsafe
programs. As an example, consider writing an abstract syntax tree for representing the
language in the compiler. The compiler writer may want to capture certain semantic
invariants on the data structure representing the abstract syntax tree (AST). For example,
the term of the conditionals on the control structures of !if! or !while! loops have to be
of !Boolean! types so that they can be evaluated to either !True! or !False!, and not result
in a runtime error, or even more complex invariants, where in a multi-stage compiler pipeline,
the AST may gain more information at every pass. This new information is stored in the AST,
and the types ensure that, if the information is populated in the third pass, trying to
access that information in the first pass will result in a typechecker
error while also ensuring maximum data structure sharing for memory efficiency.
Once we identify the right abstractions to express such complex
invariants into the type structure, we will use type theory as a mathematical tool to help us
formally reason about its correctness.
...
type !Int! and !Float! in \pref{fig:tc-num}. Typeclasses can also be viewed as relations
...
compared and added together. The declarations in \pref{fig:tc-num} can be interpreted by
...
confluence external to the system to be able to get coherence guarantees.\TODO{CHRs needs
  more explanation or move it to related section work?}
...
\pref{fig:tc-collection}. The type variable !e! stands for the element type of the
...
% The problem is that the typeclasses are relations making them too general for this use
% case.
...
\end{figure}
We surveyed the important language features, which functional
programmers like to use, and seen in some depth, a theoretical
foundation of a modern day declarative functional programming
language. The language features are tabulated in the
\pref{fig:haskell-lang-features} where The features that we have
...
dependencies in this system although they exhibit a flavor
of type equality as well. We then realized that non-parametric features
...
sophisticated proof techniques to formalize and argue their
