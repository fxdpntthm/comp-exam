...
An excerpt of typing judgements for terms is shown in \pref{fig:sfc-typing-ty}. It also does not differ much from the \SF. The novel judgement \trule{cast} transforms a term $M\co\tau$ to a term $M\co\sigma$ with a witness coercion $\Co\co\tau\sim\sigma$. The two typing judgements worth discussing are that for typing case statements $\trule{ty-case}$ and typing alternatives $\trule{ty-alt}$. A case statement can only be well typed if the discriminant has a type $\sigma$ and each of the alternatives $P \to N$ have the same type $\sigma \to \tau$. For alternatives, if the pattern is a data constructor, only the existential type variables $\many\beta$ are brought into scope explicitly in the pattern. The trickery is sound as existential quantification can be encoded using a universal quantification.
...
Inside a case term the pattern would have the form

\begin{CenteredBox}
\begin{code}
case M of IsEquals (b :: *). (d :: DEq b) (x :: b) (y :: b) -> N
\end{code}
\end{CenteredBox}

as the type of !IsEquals! is isomorphic to the one
...
eval (Plus (co :: a ~ Int) (x :: Int) (y :: Int))
 ...
 + ((eval (y /> sym co)) /> co))
 /> sym co
eval (IsEquals (co :: a ~ Bool) (d :: Eq a )
\end{code}
\end{CenteredBox}
...
We use call by name semantics as GHC uses lazy evaluation and we assume Barendregt's convention throughout the presentation.
...
\subsubsection{GADTs}\label{sec:fc-encodes-gadts}
