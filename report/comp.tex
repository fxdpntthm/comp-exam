...
typeclass. It naturally extends the idea of having n-ary relations
...
type. One can imagine having instances for such a typeclass as
\lstinline{Con Int [Int]}, !Con Int (Tree Int)!, etc. Speaking in
...
compilation. For example, in a term !t :: [Int] = insert 3 empty!, the typechecker infers
the type of !empty! as !Con e [Int] => [Int]!. The type inferred by
the typechecker is the right one. It is not enough to determine what the
...
%% The point to make here is typechecker can enforce contraints!
...
functional dependencies for multiparameter typeclasses is directly
...
As simple naive encoding via means of
