...
Scala 3~\cite{xu_implementing_2021} added them.
...
intention is to use a type function, !Elem c!, where ever
...
\begin{figure}[ht]
 ...
 \end{syntax}
 \begin{syntax}
 \text{Typing Context} &&\TEnv,\Delta \bnfeq& \empt \bnfor \TEnv,x\co\tau \bnfor \TEnv,H\co T \bnfor \TEnv,F_n\co\tau \bnfor \TEnv,\TyVar\co\kappa  \bnfor \TEnv, c \co \tau\sim\sigma\\
...
saturated in types to be well formed. The subscript $n$ specifies the
arity of the type function, but it is elided when the context makes it clear.
...
The judgements in \pref{fig:sfc-typing-co} formalize the intuitions of how coercions ought to behave. All coercions are types such that their
...
The cast transforms the scrutinee from type !List (T Bool)! to type !List Int!
...
The notation $\Left^k \Co$ stands for $\Left{}$ applied $k$ times to $\Co$.
...
we define a  multi-step reduction $\manystepsto\bullet\bullet$. It is a
...
the second question is answered by showing that the system
respects phase distinction~\cite{harper_higher-order_1989}.
...
with top level environment $\Delta = \TEnv, not\co\texttt{Bool} \to \texttt{Bool}$. If it was possible to derive !coBad! using $\Delta$
then, $f$ can be invoked, and it will
crash the program. However, if the system prohibits derivation
of such inconsistent coercions $f$ can never be called.
...
\begin{definition}[$\Good\TEnv$]
 ...
 \end{itemize}
Where a type is a Value types if it is of the form $T\many\tau$, $\tau\to\sigma$, or $\Forall{\alpha\co\kappa}{\tau\sim\sigma}$
\end{definition}
...
get stuck at runtime.  In other words, the types do not hold any
...
Erasure of terms \Erased{\Tm}, defined inductively
...
\Description{GADT Surface level type syntax}
...
In \pref{fig:gadt-type-syntax}, the constraints, $C$, are type equality
...
data GAlgExp :: * -> * where
  ...
  Plus   :: FORALL a. (a ~ Int) => GAlgExp a
                              -> GAlgExp a
                              -> GAlgExp a
  IsZero :: FORALL a. (a ~ Bool) => GAlgExp Int
                              -> GAlgExp a
\end{code}
\end{minipage}%
\begin{minipage}[ht]{0.5\linewidth}
...
eval (Value x) = x
eval (Plus (co :: a~Int) (x :: GAlgExp Int) (y :: GAlgExp Int))
   = (((eval (x /> sym co)) /> co)
     + ((eval (y /> sym co)) /> co)) /> sym co
eval (IsZero (co :: a~Bool) (x :: GAlgExp Int))
   = (isZero x) /> sym co
\end{code}
\end{minipage}
...
function !isZero! is !GAlgExp Int -> Bool!. The coercion, !co!, can
...
(existential) coercion !co : a ~ Int! exposed after the pattern is matched
and then coerced back into !Int!
...
\begin{figure}[ht]
  ...
  \Description{newtype support in \SFC}
...
replacing typeclass parameters with equal types. This is necessary to
account for associated types that appear in the typeclass
