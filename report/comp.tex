...
\begin{abstract}
  This monograph studies the practical (typechecker) and
  meta-theoretical (formalization) aspects of a
  modern, statically typed, declarative, and functional style
  programming language. Understanding the principles
  of programming language design and its implementation is crucial as they
  are the primary interface between humans and computers to encode
  unambiguous yet expressive instructions via software programs.
  The features of a typed functional programming language strive to
  provide machine verified guarantees about the programs by
  following the  ``correct by construction'' philosophy.
  We focus how a single construct, explicit type
  equality, unifies seemingly different important language
  features into a simple typed intermediate language.
\end{abstract}
...
observation in the field of computer science: Curry-Howard or the
