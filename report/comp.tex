...
\begin{figure}[ht]
  ...
  \end{syntax}
  \begin{syntax}
    \text{Kinds}     &&\kappa       &::= \star \bnfor \kappa \to \kappa \bnfor \sigma \sim \tau\\
    \text{Types}     &&\tau,\sigma  &::= \alpha \bnfor T \bnfor \tau \to \tau \bnfor \tau\App\tau \bnfor \Forall {\alpha\co\kappa} \tau \bnfor F_n\\
    \text{Coercions} &&\nu,\Co      &::= c \bnfor \refl\tau \bnfor \sym\Co \bnfor \trans\nu\Co % equiv relation
    ...
    \text{Types/Coercions} && \phi &::= \tau \bnfor \Co\\
    \text{Patterns}  &&P    &::= H\App \many{\alpha\co\kappa}\App{\many{x\co\tau}} \\
    \text{Terms}     &&M,N  &::= x \bnfor  \Lam {x\co\tau} M \bnfor M\App N \bnfor \TLam{\tau\co\kappa} M \bnfor M\App \tau \bnfor H \bnfor \Case M \many{P \to M} \bnfor \Cast \Tm \Co\\

    \end{syntax}
    \begin{syntax}
    \text{Typing Context} &&\TEnv,\Delta &::= \empt \bnfor \TEnv,x\co\tau \bnfor \TEnv,\alpha\co\kappa \bnfor \TEnv,H\co T \bnfor \TEnv, \gamma \co \tau\sim\sigma\\
    \text{Substitutions}  &&\Subst       &::= \empt \bnfor \Set{\many{\alpha \mapsto \tau}}
  \end{syntax}

  \begin{syntax}
    \text{Program} &&P_{gm} &::= \many{D_{cl}} \mathrel{;} \many{\Tm}\\
    \text{Declarations} &&D_{cl} &::= \textbf{\texttt{data}}\App T\co\many{\kappa} \to \star\App \textbf{\texttt{where}}\App \many{C_{trs}(T)} \bnfor \textbf{\texttt{type}}\App F_n : \many\kappa^n \to \kappa \bnfor  \textbf{\texttt{axiom}}\App C\App \many{\alpha\co\kappa} : \sigma_1 \sim \sigma_2 \\
    \text{Data Constructors} &&C_{trs}(T) &::= H : \Forall {\many{\alpha\co\kappa}} {\Forall {\many{\beta\co\kappa'}} \many{(\tau \sim \tau)} \then \many\sigma \to T\many\alpha}
  \end{syntax}
...
\end{code}
A type directed translation converts a source level GADT into \SFC as shown in \pref{fig:encoding-gadts}
The source level types are stratified into three groups
\begin{figure}[ht]
  \centering
  \begin{syntax}
...
\end{figure}
The type directed translation $\GTranslate C \TEnv \Tm \pi \Tm'$ says that given a surface level term $\Tm$ of type $\pi$, it is compiled to a term $\Tm'$ into \SFC under the constraints C and typing environment $\TEnv$. The key idea of the translation is that the type equality constraints, $\tau\sim\tau'$, are compiled to coercions in \SFC. The constraint C is a collection of named type equalities.
