...
functional programming language in \pref{sec:language-features}.
...
argument for its correctness. Finally, in \pref{sec:related-work} we point out some important related
work not covered here, give some directions to future
work in \pref{sec:future-work}, and make some concluding remarks in \pref{sec:conclusion}.
\end{abstract}
...
perform this transformation via a multi-step
process\cite{essentials_siek_2023}. Every useful software program
evolves over time by means of extensions. A compiler is no
...
type error. All this while also ensuring maximum data structure sharing for memory efficiency\cite{peyton_jones_trees_2017}.
Once we identify the right abstractions to express these complex
invariants in the type structure, we will use type theory
as a mathematical tool for principled reasoning of its correctness.
...
Finally, The type preservation lemma, \pref{lem:preservation-sfr},
along with \pref{lem:progress-sfr} is the formal guarantee that
