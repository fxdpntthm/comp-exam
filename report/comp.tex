...
\end{figure}
Writing an !eval! function for a !LamTm a! would be cumbersome if it were not for the constrainted type arguments of data constructors !Lam! and !App!. The local type constraints on !Lam! are necessary to convince the type checker that the application of !fun arg! is indeed well typed, as the type of !fun! is !LamTm a -> LamTm b! and the type of !arg! is !LamTm a!.

\subsection{Modules and Data abstraction}
Code modularity aids writing bug free programs. Modules help in achieveing code modularity by allowing programmers write smallish independent programs. The complete program is then a principled composition of the modules. Modules also aid in maintaining separation of concerns. by organizing related program units together and unrelated things separate. Most programming languages like Haskell, Java treat a file as a module however there are programming languages such as ML enable multiple modules within a single file.
...
\begin{figure}[ht]
  \centering
  \begin{syntax}
    \text{Value Types}  && T\Val &::= T \bnfor \tau \to \tau \bnfor \Forall {\alpha\co\kappa}\\
    \text{Plain Values} && \Val  &::= H \bnfor \Lam {x\co\tau} M \bnfor \TLam {\alpha\co\kappa} M \\
...
\begin{figure}[h]
  \centering
  \begin{syntax}
    \syntaxhl{\text{Kind Vars}} &\syntaxhl{\chi}   &                            & \\
    \text{Type Vars} &\alpha,\beta,\gamma  &\qquad\text{Type constants} &T,\syntaxhl{H} \\
    \text{Term Vars} &x,y                  &\qquad\text{Indices}        &i,n \in \mathbb{N} \\
    \text{Coercion Vars} &c & &
  \end{syntax}
  \begin{syntax}
    \text{Sort}      &&\square & \\
    \text{Kinds}     &&\kappa,\eta       &\bnfeq \syntaxhl{\chi} \bnfor \STAR \bnfor \syntaxhl{\texttt{CONSTRAINT}} \bnfor \kappa \to \kappa \bnfor \syntaxhl{T\App\many\kappa}\\
    \text{Types}     &&\tau,\sigma  &\bnfeq \alpha \bnfor T \bnfor \tau \to \tau \bnfor \tau\App\tau \bnfor \Forall {\alpha\co\kappa} \tau \bnfor F_n \bnfor \syntaxhl{\Forall \chi \tau} \bnfor \syntaxhl{\tau\App\kappa} \bnfor \syntaxhl{H}\\
    \text{Coercions} &&\nu,\Co      &\bnfeq c \bnfor \refl\tau \bnfor \sym\Co \bnfor \trans\nu\Co % equiv relation
                                      \bnfor \Forall {\alpha\co\kappa} \Co \bnfor \Co\At\tau % abstraction instanst
                                      \bnfor \nu\App\Co \bnfor \Left \Co \bnfor \Right \Co  % compose/decompose
                                      \bnfor \syntaxhl{\Forall \chi \Co} \bnfor \syntaxhl{\Co\App\chi} %
                                      \bnfor \syntaxhl{\Co\At\chi}\\
    \text{Predicates} && \phi &\bnfeq \sigma \sim \tau\\
    \text{Patterns}  &&P    &\bnfeq H\App\syntaxhl{\many\chi}\App\many{\alpha\co\kappa}\App{\many{x\co\tau}} \\
    \text{Terms}     &&M,N  &\bnfeq x \bnfor  \Lam {x\co\tau} M \bnfor M\App N \bnfor \TLam{\tau\co\kappa} M \bnfor M\App \tau \bnfor H \bnfor \Case M \many{P \to M} \bnfor \Cast \Tm \Co %
                             \bnfor \syntaxhl{\TLam \chi \Tm} \bnfor \syntaxhl{\Tm\App\kappa} \\
...
}{$$ H : \Forall {\syntaxhl{\many\chi}} {\Forall {\many{\alpha\co\kappa}} {\Forall {\many{\beta\co\eta}} {\many\tau \to T \App\many{\chi}\App\many{\alpha}}}} $$
}
...
%   \begin{codef}
%     zero : forall (a:*). TypeRep a -> a
%     zero TyInt (kco : k ~ *) (co : (a:k) ~ Int)
%         = 0 /> sym co
%     zero TyBool (kco : k ~ *) (co : (a:k) ~ Bool)
%         = False /> sym co
%     zero TyApp (kco1 : k1 ~ * -> *) (kco2 : k2 ~ *) (a:k1) (b:k2)
%                (TyList kco1 (co:(a:k1 ~ []))) _
%         = [] /> sym co
%   \end{codef}
% \end{minipage}
...
$$
\TEnv \vdash \text{(\lstinline {\\ xs -> xs == reverse xs})} : \alpha \rightsquigarrow (\alpha \sim (\gamma \to Bool)) \land (\gamma \sim [\gamma_1]) \land (\text{\lstinline{Eq}}~ [\gamma_1])
$$
...
For an ML like language, that contains lambdas ($\Lam x e$), applications ($e \App e'$), variables ($x$) and pattern matching on algebraic data types ($\Case e {\Set{\overline{K \overline x \to e}}}$ ) the rules for constraint generation are given in \pref{fig:constraint-gen}.
...
\begin{figure}[ht]
  ...
  \begin{gather*}
    ...
    \rsp
    \ib{\irule[\trule{case}]
      {\LARGE\substack{{\GenConstraints \TEnv {e} {\tau} Q} \quad {\fresh {\alpha,\overline\gamma} \TEnv} \quad {Q' = Q \land T\overline\gamma \sim \tau}\\
        {\text{For each } K_i\overline x_i \to e_i} \text{ do }\qquad\qquad\qquad\qquad\\
        (K_i\co\forall\overline{a}.~\overline\sigma_i \to T\overline{a}) \in \TEnv \quad {\GenConstraints{\TEnv, \overline{x_i:\Set{\overline{a \mapsto \gamma}}\sigma_i}} {e_i} {\tau_i} {C_i}}\\
...
$$
\SolConstraints {\Set{\forall a.~ Eq~ a \then Eq~ [a]}} {\varepsilon} {(\alpha \sim (\gamma \to Bool)) \land (\gamma \sim [\gamma_1]) \land (\text{\lstinline{Eq}}~ [\gamma_1])} {\Set{Eq~\gamma_1}} {\Set{\alpha \mapsto ([\gamma_1] \to Bool)}}
$$
...
In the \lstinline{isPalindrome} example, the simplifier could, alternatively, have returned a $Eq~[\gamma_1]$ instead of $Eq~\gamma_1$ by not using the global constraint. But, this would mean that the function would not have had the most general, or principal type. Thus, it is necessary for the simplifier to be able to compute principal types so that the type inference algorithm can infer the most general type.
...
\begin{figure}[ht]
  ...
  \begin{gather*}
    ...
    \ib{\irule[\trule{casela}]
      {\LARGE\substack{
          {\GenConstraints \TEnv e \tau C} \quad \fresh {\alpha,\overline\gamma} \TEnv\\
          ...
          {\GenConstraints {\TEnv, \overline{(x_i\co\Set{\overline{\gamma \mapsto a}}})} {e_i} {\tau_i} {C_i}} \qquad {\overline{\delta_i} =  fv(ruben sandwitch) - fv}
        }};
