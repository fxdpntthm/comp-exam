...
\item[Part I] serves as a reminder of the essential features
  ...
  \pref{sec:language-features}. This gives the \emph{programmers'
    view} of the language motivated using real world examples.
\item[Part II] formalizes \SFC in the \pref{sec:sfc}. It is in essence
  ...
  system in the \pref{sec:sfc-encoding-features}.
\item[Part III] describes three important extensions of vanilla \SFC in
  the \pref{sec:fc-extensions}. As, \SFC is insufficient to encode all the
...
\item[Part IV] describes notable open problems in the \pref{sec:future-work}.
  To be comprehensive, it is followed by related work in the
  \pref{sec:related-work}, and finally the conclusion is in the
  \pref{sec:conclusion}.
\end{enumerate}
...
system\cite{milner_theory_1978}. Implicit
parametric\cite{reynolds_user-defined_1978}, and adhoc
...
means that at the function use site explicit types need to be applied
before their value arguments can be applied.
For example, the function !id! in \SF will have a type
...
Elaboration of algebraic datatypes in \SF have also been widely
studied in literature; various encodings schemes such as
Church\cite{jansen_efficient_2005,jansen_programming_2013},
...
to elaborate user defined datatypes in \SF.
The data constructors introduced by user defined datatypes are treated as
term constants with the appropriate argument and type specified by the
declaration. For example, consider the datatype declaration of
!AlgExp! from before. The data constructor !Value! has the type
!FORALL a. a -> AlgExp a! in the core language, and programmer written
term !Value 3! will be elaborated to !Value Int 3!. Notice how the
types need to be explicitly applied in the core language. The
elaboration procedure needs to find unique values to fill into the
placeholder arguments for core level functions. Typechecking
expressions that contain value constructors of user defined algebraic
datatypes is as straightforward as type checking simple term
applications as the type of each value constructor can be inferred
...
Consider an associated datatype type !Elem [Int]! from the
...
It is evident that with the addition of GADTs, generative
types, and associated types using orthogonal extensions of pure \SF
would be too cumbersome, if  not impossible to express. Further, the
interaction of each of these seamy different features together would
be painful for the compiler writer. \SFC\cite{sulzmann_system_2007}
was thus designed as an alternative to having separate extensions for
each language feature. The key insight is that each language feature is in a sense
...
`$\Co_1\Co_2$', and `$\Left\Co$' and `$\Right\Co$' constructs
...
be introduced as equality axioms which can be thought as a templatized
