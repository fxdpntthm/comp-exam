...
In the rule \trule{co-nth} is important to have coercion decomposition to occur only on type constants that are not generative types. For example consider the following definitions
...
With the above rules, we assign a phantom role for the type parameter !a! of the data type !Any! as intuitively, its instantiation does not influence how the value term !Any! is represented in memory. Now consider the newtype !App!, the roles inferred will be $\roles{\texttt{App}} = R, N$. Now, due to the rules of the coercion calculus we can derive the following representational type equality:
...
\end{align}
The first step (and the last step) of the reasoning is valid due to the instantiations of the coercion axiom !coApp : FORALL f a. App f a $\teqR$ f a!, which rises by virtue of the definition of the newtype. We do not want to allow the rule \trule{co-nth} to \pref{eqn:app-any-newtype}. Because if we did, then we would be able to produce a coercion $\texttt{Int} \teqR \texttt{Bool}$, a blunder.
...
\begin{lemma}[Progress \SFR]\label{lem:progress-sfr}
 If $\Good \TEnv$ and $\Typing \TEnv \Tm \tau$ then, either $\Tm \in C\Val$ or, $\stepsto \Tm \Tm'$
\end{lemma}
The proof of preservation uses the formulation of coercion calculus extended with roles and thus requires some attention. The reason for type unsoundness was that we allowed too many coercions with unrestricted coercion lifting in \SFC. The \pref{lem:preservation-sfr} is the formal guarantee to ensure that type preservation gives us the type safety guarantees along with \pref{lem:progress-sfr}.
\begin{lemma}[Preservation \SFR]\label{lem:preservation-sfr}
...
\begin{figure}[ht]
 ...
 \hline
 Modules\cite{macqueen_modules_1984}    & Typeclasses\cite{wadler_polymorphism_1989} (and Functional Dependencies\cite{jones_tcfd_2000})\\
 Generative abstract types   & Type Functions (Associated types\cite{chakravarty_associated_2005}, Open\cite{schrijvers_type_2008}, Closed\cite{eisenberg_typefamilies_2014})\\
 Algebraic Datatypes\cite{burstall_hope_1980}         & Generalized Algebraic Datatypes\cite{cheney_first-class_2003}\\
 \end{tabular}
 \caption{Features of Haskell}
 \label{fig:haskell-lang-features}
\end{figure}
\AI{make this table fit on a page}
