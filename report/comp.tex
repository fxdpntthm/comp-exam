...
\begin{abstract}
This report is a juxtaposition of practical (type checker) and meta-theoretical (formalization) aspects of statically typed functional programming language design. It is organized into six main sections. After an eagles eye view\pref{sec:introduction} which sets the scope of problem space, in \pref{sec:language-features}, we go through some quintessential and some advanced language features. Some of these are specific to Haskell. This serves as a \emph{programmers view} of the language. The \pref{sec:sfc} focuses on formalization of \SFC, which is in essence, \SF enriched with first class explicit type equalities. Adding type equalities as a core language construct has extensive repercussions. They efficiently encode a diverse set of language features which we sketch in \pref{sec:sfc-encoding-features}. These features were previously either impossible or required non-trivial extensions to \SF making the overall system convoluted and complex to maintain. This portion of the report is the \emph{compiler writers} view of the language. In \pref{sec:fc-extensions}, we pose different extensions to \SFC; vanilla \SFC is not sufficient to encode all programming idioms which functional programmers would want to express: \SFR makes type equality finer grained, \SFP makes the kind system more expressive, and, \SFK enables expressing kind level equalities by squashing the distinction between types and kinds. Finally, in \pref{sec:related-work} we conclude by pointing out some important related work and notable open problems.
\end{abstract}

\maketitle
\section{Introduction}\label{sec:introduction}
Constructive mathematics and computer programming are acts of doing the same thing: identifying and exploiting the right abstractions to solve a problem. Mathematicians tackle it from top to bottom: by building the abstractions first and then enriching them, to make them represent real world problems, while programming turns the process on to its head, the programmer first writes a program, possibly incorrect, to solve a very specific task, and then by means of iterative code refactoring, build the right level of abstraction, while possibly fixing the incorrect program behaviors. This observation is well established and even has a name, Curry-Howard or the Bouwer-Heyting-Kromoglov correspondence\cite{TODO} in the field of Type Theory\cite{TODO}. A compiler is nothing but a software that transforms a higher level language suitable for humans (programmers) to a lower level language (suitable for computers) possibly via a series of transformations. Every software evolves and so does the compiler software and every new language feature is implemented as a new compiler feature. There are two ways of doing this extension: encode the new language feature by using existing the high level language features. In this encoding scheme, we just define a high level language to a high level language syntax transformer. Some exotic language features may not be able to leverage this encoding scheme and for such cases, we need to enrich the intermediate language along with the higher level language. However, multiple such extensions cause a clutter in the intermediate language, making it difficult to maintain. We need to find the right abstractions to refactor the intermediate language so that all different high level language features can be encoded precisely into a unified intermediate language with fewer extensions and easy to maintain.

In compilers for a statically typed programming languages, the type checker is the gate keeper. It needs to ensure that it does not let ``bad'' programs to pass. This allows the programmers to liberate themselves from a class of programming errors, such as null pointer exceptions due to incorrect function arguments, that result in runtime errors. On the other hand, the type checker also needs to ensure that programmers can express all the programs that they want write. Type theory is a mathematical framework that helps us identify the right abstractions and also aid formal reasoning, to design and implement type checkers. The structure of types needs to be sufficiently enriched to express complex programmer ideas while rejecting runtime unsafe programs. As an example, consider writing an abstract syntax tree for representing the language in the compiler. The compiler writer may want to capture certain semantic invariant on the data structure representing the abstract syntax tree (AST). For example, the term of the conditionals on the control structures of !if! or !while! loops have to be of Boolean types so that they can be evaluated to either !True! or !False! and not result in a runtime error, or even more complex invariant where in a multi stage compiler pass, the AST gains more information at every pass. This new information is stored in the AST and the types ensure that if the information is populated in the third pass, trying to access that information in the first pass results in a type checker error and not a runtime error.
...
\TODO{How do i center align these code blocks?}
The function !id!, shown above, abstracts over all types by an implicit universal quantification over the type variable !a!. Such functions are called parametrically polymorphic\cite{strachey_fundamental_2000}. The compiler can deduce which instance of the polymorphic function is required when an argument of a concrete type is passed as an argument to a polymorphic function. Other imperative languages such as Java and C also allow user defined polymorphic functions but often require explicit pointer casts or the use of interfaces. Parametric polymorphism also enables first class functions via higher order functional programming. Functions can be passed as arguments to other functions. For example, the type of the left hand side !id! in the definition of !t! will be assigned the type !(a -> a) -> (a -> a)! by the typechecker. ML\cite{milner_logic_1975, milner_theory_1978} was the first functional programming language to introduce declarative parametric polymorphism.
...
\begin{figure}[ht]
 ...
 \hline\hline
   ...
   \hline
   \multirow{4}*{\emph{Generative abstract types}\cite{breitner_safe_2014}} & \emph{Type Functions}\\
                                        & \emph{Open}\cite{schrijvers_type_2008}\\
                                        & \emph{Closed}\cite{eisenberg_typefamilies_2014},\\
...
In this report, we surveyed, in breadth, the different important language features which functional programmers like to use and, in depth, a theoretical foundation of a modern day declarative functional style programming language. The language features that we have covered in this report in some detail are emphasized in \pref{fig:haskell-lang-features} along with other language features. There are two ways of supporting programming language features, either via encoding them with the help of libraries or, via supporting them as a core language feature. In the latter case, we realized that composing different language features is non-trivial as their subtle interactions can threaten language inconsistency. We first recognized that the three seemingly different language features: generative abstract types, generalized algebraic datatypes and various flavors of type functions can be encoded using a single construct: type equality. However, there is no satisfactory account of functional dependencies in this system even though they also exhibit some flavor of type equality. We then identified that non-parametric features interacting with parametric features of the language that contain explicit type equalities causes type unsoundness bugs unless they are treated carefully. Taming ad-hoc features is non-trivial and requires sophisticated proof techniques to argue their correctness. As new programming languages are designed, built and used, it is not only important to identify and carry over the good design principles established by theory using the formalization techniques, but also strengthen the foundational theory by understanding the requirements of practice.

Open problems and future direction: Are we falling in the same of cramming the language pit by adding equalities. give more examples.
