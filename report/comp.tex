...
\begin{figure}[ht]
  ...
  \end{syntax}
  \begin{syntax}
    \text{Kinds}     &&\kappa       &::= \star \mid \kappa \to \kappa \mid \sigma \sim \tau\\
    \text{Types}     &&\tau,\sigma  &::= \alpha \mid T \mid \tau \to \tau \mid \tau\App\tau \mid \Forall {\alpha\co\kappa} \tau \mid F_n\\
    ...
    \text{Types/Coercions} && \phi &::= \tau \mid \Co\\
    \text{Patterns}  &&P    &::= H\App \many{\alpha\co\kappa}\App{\many{x\co\tau}} \\
    \text{Terms}     &&M,N  &::= x \mid  \Lam {x\co\tau} M \mid M\App N \mid \TLam{\tau\co\kappa} M \mid M\App \tau \mid H \mid \Case M \many{P \to M} \mid \Cast \Tm \Co\\

    \end{syntax}
    \begin{syntax}
    \text{Typing Context} &&\TEnv,\Delta &::= \empt \mid \TEnv,x\co\tau \mid \TEnv,\alpha\co\kappa \mid \TEnv,H\co T \mid \TEnv, \gamma \co \tau\sim\sigma\\
    \text{Substitutions}  &&\Subst       &::= \empt \mid \Set{\many{\alpha \mapsto \tau}}
  \end{syntax}

  \begin{syntax}
    \text{Program} &&P_{gm} &::= \many{D_{cl}} \mathrel{;} \many{\Tm}\\
    \text{Declarations} &&D_{cl} &::= \textbf{\texttt{data}}\App T\co\many{\kappa} \to \star\App \textbf{\texttt{where}}\App \many{C_{trs}(T)} \mid \textbf{\texttt{type}}\App F_n : \many\kappa^n \to \kappa \mid  \textbf{\texttt{axiom}}\App C\App \many{\alpha\co\kappa} : \sigma_1 \sim \sigma_2 \\
...
\begin{figure}[ht]
  ...
  \begin{syntax}
    ...
    \text{Evaluation Contexts} &&  \EvalCtxt &::= \EvalCtxtHole{-} \mid \EvalCtxt\App M \mid \EvalCtxt \tau \mid \Cast \EvalCtxt \Co \mid \Case \EvalCtxt {\many{P}}\\
  \end{syntax}
  \begin{gather*}
    \fbox{$\stepsto M N$}\\
    \Beta \rsp \TBeta\\
...
Armed with the new coercion infrastructure, it is now possible to encode most of the features discussed in \pref{sec:language-features}. \SFC is a conservative extension of \SF, so type classes and algebraic datatypes can be encoded in \SFC as is. GADTs and associated types demand some more details.
