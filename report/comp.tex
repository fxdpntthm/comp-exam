...
\item In an AST where the compiler writer wants to enforce the
  ...
  representing the condition is of !Boolean! type.
...
evaluated values of the !AlgExp! into a datatype? This certainly
...
dependencies: we do not want the compiler to make arbitrary decisions
for the programmer.
...
(!Elem [Int] ~ Int!). The generative abstract type introduces
type equalities between the new defined type and its representation.
(!Html ~ String!).
% By introducing a single construct for type equality
...
\fbox{$\CoKinding \TEnv \Co \kappa$} as, ``under the assumptions in
$\Gamma$, the coercion $\Co$ provably is of kind $\kappa$''. The kinding rules
...
coercion term, but a data constructor. The rule \trule{case}
can now be applied to make progress in evaluation.
...
derivable within the system, any term that is of type !Int! could be
...
\begin{corollary}[Syntactic Soundness]\label{lemma:sfc-syntax-soundness}
  ...
  then $\manystepsto \Tm \Val$ iff $\manystepsto {\Erased\Tm} v$, where $v\in\Erased\Val$, or $\Tm$ diverges.
\end{corollary}
...
optimization step cannot be performed on algebraic datatypes declared
...
\end{code}
\end{minipage}
\caption{Vector datatype with typelevel length}
\label{fig:vec-star}
\end{figure}
...
vector of size !n! to return a vector of size !S n!. In this system,
there is no enforcement that the type argument to !Vec!
has to be either !Z! or !S n!. Any type of kind, $\star$ is a valid
argument. A desirable alternative would be to restrict the kind
of the argument to only those elements that can describe the size of the vector.
In this setting, as shown in \pref{fig:vec-nat},
the typechecker now has enough information to reject
...
\end{code}
\end{minipage}
\caption{Vec with Nat index}
\label{fig:vec-nat}
\end{figure}

Similar problems arise while writing an addition
function on type level naturals. Type functions do not include
any kind information; the argument and return types are all defaulted
to kind $\star$. The intention of the programmer is to only allow very specific
types to appear in the position of argument and return types. However, the
system is not expressive enough to do it. The type !Plus Int Char! is well
kinded term, but semantically absurd. A more expressive system
would allow types of specific concrete kinds to the type function.
In this case specifying that the kind of the argument and return
types of !Plus! is a !Nat!.
...
In essense, the extension to the type system is to make
the kind level more expressive. The system can now have more than
one concrete kind, $\star$. To get more power out of a system which
has more than one base kind is to enable kind polymorphism.
Consider a higher kinded datatype !TApp! which is useful
for generic programming that encodes type application~\cite{magalhaes_generic_2010}:
...
What should be the kind of the type !TApp!?
A naive algorithm may assign the first type argument, !f!, to be of kind $\star \to \star$,
and the second type argument !a! to be of kind $\star$. This would assign the
kind of type !TApp! to be $(\star \to \star) \to \star \to \star$. The
type !TApp! can now be used for encoding type applications such as
the type !List Char! using the type !TApp List Char!.
However, this kind is too restrictive and will disallow encoding other interesting datatypes.
For example, !Vec Z elem!. Its type level encoding will be !TApp (TApp Vec Z) Char!.
It will be ill-kinded in this system due to two reasons: (1) the kind of !TApp! is incapable of
encoding type applications for types that have more than one type
argument---!Vec! takes two type arguments---, and (2)
the type arguments can only be of a concrete kind $\star$---the first type argument to !Vec!
is of kind !Nat!. A more appropriate kind for !TApp! would be $\Forall k {(k\to\star) \to k \to \star}$.
This would allow a wider range of types to benefit from generic programming techniques.
There are two requirements to make the type (kind) system expressive enough to allow
programmers to write the programs which use !Vec! type shown above. First,
the system needs to allow users to define kinds and second, the kind inference
needs to account for polymorphism at kind level.

\subsection{Syntax}\label{sec:sfp-syntax}
\begin{figure}[ht]
 \centering
 \begin{syntax}
 \shl{\text{Kind Vars}} &\shl{\KVarX, \KVarY} &  & \\
 ...
 \text{Kinds} &&\kappa,\eta \bnfeq& \star \bnfor
                                  \kappa \to \kappa \bnfor \sigma\sim\tau \bnfor \shl{\KVarX} \bnfor \shl{T\App\many\kappa}\\
 \text{Types} &&\tau,\sigma \bnfeq& \TyVar \bnfor T
                                  \bnfor \tau \to \tau \bnfor
                                  \tau\App\tau \bnfor \Forall {\TyVar\co\kappa} \tau  \bnfor F\many\tau \bnfor \Co
                                  \bnfor \shl{\Forall \KVarX \tau} \bnfor \shl{\tau\App\kappa} \bnfor \shl{H}\\
 \text{Coercions} &&\nu,\Co \bnfeq& c \bnfor \Refl\tau \bnfor \Sym\Co \bnfor \Trans\nu\Co % equiv relation
 \bnfor \Forall {\TyVar\co\kappa} \Co \bnfor \Co\At\tau % abstraction instanst
 \bnfor \nu\App\Co \bnfor \Left \Co \bnfor \Right \Co  % compose/decompose
 \bnfor \shl{\Forall \KVarX \Co} \bnfor \shl{\Co\App\KVarX} %
 \bnfor \shl{\Co\At\KVarX}\\
 \text{Patterns} &&P \bnfeq& H \App\many{\beta\co\kappa}\App\App\many{x\co\tau} \\
 \text{Terms} &&M,N \bnfeq& x \bnfor \Lam {x\co\tau} M \bnfor M\App N \bnfor \TLam{\tau\co\kappa} M \bnfor M\App \tau \bnfor H \bnfor \Case M \many{P \to M} \bnfor \Cast \Tm \Co %
 \bnfor \shl{\TLam \KVarX \Tm} \bnfor \shl{\Tm\App\kappa} \\

 \end{syntax}
 \begin{syntax}
 \text{Typing Context} &&\TEnv,\Delta \bnfeq& \empt \bnfor \TEnv,x\co\tau \bnfor \TEnv,\TyVar\co\kappa \bnfor \TEnv,H\co T \bnfor \TEnv, \Co \co \tau\sim\sigma\\
 \text{Substitutions} &&\Subst \bnfeq& \empt \bnfor \Set{\many{\TyVar \mapsto \tau}}\\
 \text{Data Constructors} &&C_{trs}(T) \bnfeq& H : \Forall {\App\shl{\many\KVarX}} {\Forall {\many{\TyVar\co\kappa}} {\Forall {\App\shl{\many\KVarY}} {\Forall {\many{\beta\co\kappa'}} \many\sigma \to T\many\TyVar}}}
 \end{syntax}
 \caption{The Syntax of \SFP as an extension for \SFC}
 \label{fig:sfp-syntax}
\end{figure}

\SFP~\cite{yorgey_giving_2012}, achieves both the requirements mentioned above
with a relatively small conservative extension to \SFC.
The syntax for the core language is shown in \pref{fig:sfp-syntax}.
The key changes in the language of types---highlighted to compare with \SFC---are
allowing kind variables ($\chi$), kind abstraction ($\Forall \chi \kappa$),
and kind application ($\tau\App \kappa$). These changes are reflected
in the coercion language which needs additional constructs:
kind polymorphic congruence ($\Forall \chi \Co$),
kind application ($\Co\App\chi$) and kind instantiations ($\Co\At\kappa$).

Other noteworthy addition to the kind and type level language are
promoted constructors $T\App\many\kappa$ and $H\App\many\tau$, respectively.
The data constructors are now explicit in their kind polymorphism.
As evident from the type signatures, they expect all the kind variables, $\many\chi$, to be instantiated
before any type variables are instantiated.
$$
H : \Forall {\shl{\many\KVarX}} {\Forall {\many{\TyVar\co\kappa}}
    {\Forall {\shl{\many\KVarY}}  {\Forall {\many{\beta\co\eta}} {\many\tau \to T
      \App\many{\chi}\App\many{\TyVar}}}}}
$$
The kind variables in $\KVarX$ abstract over the universal kind variables,
while the kind variables in $\KVarY$ are abstracted over the existential ones.
Formally, the free variables in $\kappa$ must appear in $\many\KVarX$ and the
free variables in $\eta$ must appear in $\many\KVarY$.
For example, the data constructor !MkTApp!, in this system will have following type:
$$
\texttt{MkTApp} :: \Forall \kappa {\Forall {f\co\kappa\to\star}
        {\Forall {a\co\kappa} {f\App a \to \texttt{TApp}\App \kappa\App (f \App a)}}}
$$
!MkTApp! has only universal kind variables and no existential kind (or type) variables.
To capture promotion of datatypes to kind level,
the kind level syntax now has $T\many\kappa$ construct.
The simplest example is that of the datatype !Nat!, defined in \pref{fig:vec-nat},
where !Nat! is now classified as a valid kind. Similarly, the promotion of data constructors
is captured by $H$ at type level. The data constructor !Z! is a type of kind !Nat!, and similarly
!S! is of kind !Nat -> Nat!. For technical reasons,
the syntax classification hierarchy \emph{all} kinds
are classified by a unique sort, $\square$.
Finally, to keep the kind inference tractable, there are no kind level lambdas,
only promotion of higher kinded types which would stand in for kind functions.

\subsection{Static Semantics}

\newcommand\KLVar{
  \ib{\irule[\trule{V-Lift}]
     {\alpha \mapsto \KVarX \in \TEnv};
     {{\Promote \TEnv {\alpha} {\KVarX}}}
  }
}

\newcommand\KLTConst{
  \ib{\irule[\trule{T-Lift}]
     {\Kinding \empty T {\many\star \to \star}}
     {\many{{\Promote \TEnv \tau \kappa}}};
     {{\Promote \TEnv {T\many\tau} {T\many\kappa}}}
  }
}

\newcommand\KLAbs{
  \ib{\irule[\trule{Abs-Lift}]
     {\Promote{\TEnv, \alpha \mapsto \KVarX}{\tau}{\kappa}};
     {{\Promote \TEnv {\Forall {\alpha\co\star} \tau} {\Forall\KVarX\kappa}}}
  }
}


\newcommand\KLiftH{
  \ib{\irule[\trule{K-Lift}]
     {H\co\tau \in \TEnv}
     {{\Promote \empty \tau \kappa}};
...
\newcommand\KCoVarAx{
 \ib{\irule[\trule{co-$\Co$-ax}]
   {\substack {\mathlarger{c\co \Forall{\many\KVarX}{\Forall{\many{\alpha\co\eta}}{\tau'\sim\tau''}} \in \TEnv}\\
              {\mathlarger\Subst_\kappa = \Set{\many{\KVarX \mapsto \kappa}},
...
\newcommand\KCoKappaInst{
 ...
 {\Subst = \Set{\KVarX \mapsto \kappa}}{\Kinding \TEnv \kappa \square}
 {\CoKinding \TEnv {\Co} {\Forall\KVarX\tau \sim \Forall\KVarX{\tau'}}};
...
\newcommand\KKTyAbs{
  \ib{\irule[\trule{ty-$\I{\forall\KVarX}$}]
     {\Kinding {\TEnv, \KVarX\co\square} {\tau} {\star}};
     {\Kinding \TEnv {\Forall{\KVarX}{\tau}} \star}
  }
}

\newcommand\KKTyApp{
  \ib{\irule[\trule{ty-$\E{\forall\KVarX}$}]
     {\Kinding \TEnv \tau {\Forall \KVarX \kappa}}
     {\Typing \TEnv \eta \square}
     {\Subst = \Set{\KVarX \mapsto \eta}};
...
\newcommand\TyKAbs{
  \ib{\irule[\trule{$\I\kappa$}]
     {\Typing {\TEnv,\KVarX\co\square} \Tm \tau};
     {\Typing \TEnv {\TLam \KVarX \Tm} {\Forall \KVarX \tau}}
  }
}

\newcommand\TyKApp{
  \ib{\irule[\trule{$\E\kappa$}]
     {\Kinding \TEnv \Tm {\Forall \KVarX \tau}}
     {\Kinding \TEnv \kappa \square}
     {\Subst = \Set{\KVarX \mapsto \kappa}};
...
\newcommand\TyCase{
  \ib{\irule[\trule{case}]
    {\Typing \TEnv \Tm {T\App\many\kappa\App\many\sigma}}      {\many{H\co\Forall{\many\KVarX}{\Forall{\many{\alpha\co\kappa}}{\Forall{\many{\KVarY}}{\Forall{\many{\beta\co\eta}}{\many\tau \to T\App\many\KVarX\App\many\alpha}}}} \in \TEnv}}
     {\substack{\mathlarger{\Subst_\kappa = \Set{\many{\KVarX \mapsto \kappa}}}\\
               {\mathlarger{\Subst_\sigma  = \Set{\many{\alpha \mapsto \sigma}}}}}}
     {\Typing {\TEnv, \many{\KVarX\co\square},\many{\beta\co\Subst_\kappa\eta}, \many{x\co\Subst_\kappa\Subst_\sigma\tau}} u \tau};
     {\Kinding \TEnv {\Case \Tm {\many{H\App\many{\KVarY}\App\many{\beta\co\eta}\App\many{x\co\tau} \to u}}} {\tau}}
  }
}
...
\begin{figure}[ht]
  \centering
\begin{gather*}
  \fbox{$\Promote\TEnv\tau\kappa$}\\
  \KLVar \rsp \KLTConst \rsp \KLAbs\\
\end{gather*}
\begin{gather*}
  \fbox{$\Kinding \TEnv \tau \kappa$}\\
  \KLiftH \rsp \KKTyAbs \rsp \KKTyApp\\
\end{gather*}
 \begin{gather*}
 \fbox{$\CoKinding \TEnv \tau \kappa$}\\
...
While simple algebraic datatype declarations, like !Nat!, can be promoted in a straightforward fashion,
promotion higher kinded datatypes, like !Vec!, seems to be non-trivial.
The first argument to !Vec! will need to be of sort !Nat!. However, this system,
is only one sort, $\square$. This is a conscious design decision to keep the
extension minimal. \SFP is a conservative extension to \SFC which
ensures that the usability and backwards compatibility
of the extended system is maintained: the data type declarations
which the system previously accepted, should still be accepted
and the inability to promote should not cause the type checker
to reject programs that previously were accepted.
\SFP uses the following criteria for promotion:

\begin{itemize}
  \item For a type constant, $T\App\many\kappa$ can be promoted to kind level
      only if $T$ is fully saturated and is of kind $\many\star\to\star$
\item For a data constructors, $H\App\many\tau$ can be promoted to a type level
      only if all the type arguments, $\many\tau$, to the constructor
      can be promoted and the type of the data constructor can be promoted.
\end{itemize}

The promotion of a type to kind, and a term--data constructor--to type is given
by $\Promote \TEnv \tau \kappa$ rules as shown in \pref{fig:sfp-static-sem}.
They specify exactly when a type can be promoted
to a kind. In the $\trule{Abs-Lift}$ the type polymorphism is lifted to kind polymorphism.
The rule $\trule{K-Lift}$ uses promotion to ensure that a type is well kinded.



\subsection{Operational Semantics}\label{sec:sfp-opsem}
% \TODO{Does adding kind abstraction and kind application business to
%   terms meddle with the operational semantics? we need to make sure we
%   can erase them and preserve the phase distinction property.}

Most of the complexity in \SFP is inherited from \SFC, the only rule
that needs attention is the rule for the match scrutinee which is
a cast. We need to be able to push the cast inside the data
constructor pattern match so that it is no longer a coercion value, but a
term value. It can then get ``unstuck'' and leverage simple pattern match
operational semantics as shown in $\trule{H-Push}$.
For the evaluation of case expression that matches on the pattern, $\Ptrns$,
the continuation expression needs to substitute the type and kind instantiations, along
with the term substitutions.


\newcommand{\HPushSFP}{
 \ib{\irule[\trule{h-push}]
 {\substack{\mathlarger{\Co : T\App\shl{\many\kappa}\App\many\sigma \sim T\App\shl{\many\kappa}\App\many\tau}\\
\mathlarger{H\co\Forall{\many\KVarX}{\Forall{\many{\alpha\co\kappa}}{\Forall{\many{\KVarY}}{\Forall{\many{\beta\co\eta}}{\many\tau \to T\App\many\KVarX\App\many\alpha}}}}}}}
 {\substack{\mathlarger{{\Subst = \Set{\many{\TyVar_i \mapsto \Co_i}, \many{\beta_i \mapsto \phi_i}}}}\\
 \mathlarger{\Tm'_i = \Cast {\Tm_i} \Subst\rho_i}\\
 \mathlarger{\Co_i = \Right (\Left^{i-1}\Co)}}}
 {\phi' = \begin{cases}
           \Cast {\phi_i} \Subst(v_1 \sim v_2) &\text{if }\beta_i:v_1 \sim v_2\\
           \phi_i\quad &\text{otherwise}
          \end{cases}};
 {$\stepsto {\Case {(\Cast {H\App \many\kappa\App \many\sigma\App
         \many{\kappa'}\App \many\phi\App \many\Tm} \Co)} {\many{\Ptrns \to N}} }
 {\Case {(H\App \many\kappa\App\many\tau\App\many{\kappa'}\App\many{\phi'}\App\many{\Tm'})} {\many{\Ptrns \to N}} }$}
 }
}
...
\HPushSFP
\end{gather*}
\caption{Excerpt of Operational Semantics of \SFP}
\label{fig:sfp-op-sem}
\end{figure}

\subsection{Meta-theory}\label{sec:sfp-metatheory}
The meaning type equality in this system is same as \SFR: If two types are equal
then their kinds have to be the same. This design decision keeps the system tractable.
as there are no equalities between kinds nor kind coercions in \SFP.
There are two main theorms to prove that the system is type safe: Progress and Preservation.
There are two key definitions needed before characterizing the type safety.

\begin{definition}[Value type]
   A type $\tau$ is a value type when it is well kinded, $\Kinding \TEnv \tau \kappa$,
   and it is of the form $T\many\kappa\sigma$,
 $\Forall{\alpha\co\kappa}\sigma$, or $\Forall\KVarX\sigma$

\end{definition}
\begin{definition}[$\GoodP\TEnv$]
 A type environment, $\TEnv$, is $\GoodP\TEnv$ when it satisfies the
 following properties:
 \begin{itemize}
 \item If $\CoKinding \TEnv \Co {T \many\sigma \sim \tau}$ and $\tau$
   is a value type, then $\tau$ is of the form $T\App\many\sigma'$
 \item If $\CoKinding \TEnv \Co {\Forall {\TyVar\co\kappa} \sigma \sim
     \tau}$ and $\tau$ is a value type, then $\tau$ is of the form
   $\Forall {\TyVar\co\kappa} \sigma'$
 \item If $\CoKinding \TEnv \Co {\Forall {\KVarX} \sigma \sim
     \tau}$ and $\tau$ is a value type, then $\tau$ is of the form
   $\Forall {\KVarX} \sigma'$
 \end{itemize}
\end{definition}
...
The definition of $\GoodP\TEnv$ is very similar to the definition of $\Good\TEnv$ but
now it needs to account for kind polymorphism.

Finally the progress and preservation theorems:
\begin{theorem}[Preservation for \SFP]\label{lem:sfp-preservation}
 If $\TEnv$  is a top level environment and $\GoodP\TEnv$ then if $\Typing \TEnv \Tm \tau$ and $\stepsto \Tm \Tm'$,
 then $\Typing \TEnv \Tm' \tau$
\end{theorem}
\begin{theorem}[Progress for \SFP]\label{lem:sfp-progress}
If $\TEnv$ is a top level environment, $\GoodP\TEnv$, and $\Typing \TEnv \Tm\tau$ then either e is a value or a coersion value, or there exists a term $\Tm'$ such that $\stepsto \Tm \Tm'$.
\end{theorem}

The proof does not require any ingenuity than proving type safety for \SFR or \SFC.
The theorems do need to account for kind substitutions with term substitutions.

% This keeps the design and implementation simple but at
% the expense of leaving out certain programs that can be well
% typed. This gives us some confidence over the claim that we should also be able to carry over the type
% safety results from \SFC. A logical continuation of kind abstraction
% and application would be introduce kind equalities within the
% system. This will help in making the type system even more expressive.

% \subsection{Practical Aspects}\label{sec:sfp-practical}
% Due to the presense of non-trivial kinds in the system,
% The type system implementer needs to make a design decision of
% whether the programmer is expected to provide minimal (if any)
% kind annotations to the programs or the type system can infer all
% the kind information. In either case, the type checking algorithm needs
% to change. One design decision is to kind generalize exactly
% at those places where types are generalized. However, while
% computing kind unifications during kind instantiations, there
% are no evidences generated. Type equalities, on the other hand,
% generate evidences which are witnessed by coercions.
% However due to the restricion imposed by the type system---there are no
% kind non-syntactic equalities---kind equalities get no evidences.
% This helps in performing kind unification on the fly.
...
% \item The kind inference assigns a new unification kind variable
%       to each of the type constant and then walks over the definitions
%       solving for kind constraints that may arise;
...
% \AI{Motivation Needs a rewrite. Pushing $\tau \sim \sigma$ to the level of kinds
% makes sure that phase distinction is maintained.}
% While \SFP provided a lightweight method to make the system more expressive
% that \SFC by introducing a non-trivial kind system, the equality judegments can only be made
% at kind level. The only way to introduce a coercion $\tau \sim \sigma$ is by defining a
% type family and then instantiations of the induced axioms will give rise to the needed equalities.
% Consider a GADT example below which encodes the representation of types:

% \begin{CenteredBox}
% \begin{code}
% data TyRep :: TYPE -> TYPE where
%     TyInt :: a ~ Int => TyRep a
%     TyBool :: a ~ Bool => TyRep a
% \end{code}
% \end{CenteredBox}
% The intension of the user is to fix the type of each of the data constructors. However,
% in the formalization this definition is syntactically illegal in \SFP. There is no syntactic notion of
% $a \sim \texttt{Int}$ at the level of types. It would be beneficial to empower
% the programmers to express equalities at the level of types rather than at the level of kinds.
% Consider the function !zero! as defined below:

% \begin{CenteredBox}
% \begin{code}
% zero :: forall a. a ~ Int => TyRep a -> a
% zero TyInt = 0
% \end{code}
% \end{CenteredBox}
% The function !zero! is well typed only when !a! is of type !Int! and it returns !0! when applied to !TyInt!.
% The term !zero TyBool! is ill-typed. The definition of !zero! is also ill-formed in \SFP.
...
As an illustration of how the lifting context aids the push rule,
...
However, \SFK is no more inconsistent than \SFC, where all kinds are are inhabited.
The typechecking problem for \SFK is decidable. Neither does inclusion of this judgement
causes any undecidability problems for type checking, as all the terms are explicitly typed,
and the type equalities are always accompanied with a finite equality proof.
At the heart of the \SFC, which allows programmers write interesting programs by enabling encoding
complex type (and kind) equalities, lies an important question: what exactly does $\tau \sim \sigma$
mean? The proposition $\tau \sim \sigma$ says that the types $\tau$ and $\sigma$ are
equal and their kinds are equal. The system also maintains a distinction between the (equality)
proofs and objects. This allows proofs to be independent of any computational value.

It is important to note that the type in type axiom is not included in the source level language.
\SFK is designed to be used as an internal language of the compiler and there is an
evident gap between the expressivity of the source level language to the internal language.

The following meta-theory of the system proceeds in a routine fashion via showing that the system
enjoy the property of progress---well typed terms are either a value or they can evaluate---, and
preservation---evalutation does not affect the type of the term.
The progress property is uninteresting, however, the proof of preservatioin is more complex.
The previous proof techniques used to argue about consistency are ill-suited for this system
due to the involvement of kind level equalities.

We have the following properties by the way of static semantics.
...
The lifting operation, $\Lift -$, plays an important role in defining the rule $\trule{S-KPush}$. This rule
plays an important role to establish the invarient property of the system: coercions do not
interfere with the operational semantics of the terms.
...
From the previous section on \SFP, we can now also relax some of the contraints on promotion.
This restriction on promotions of type constants is because promoting higher kinded types, such as $(\star \to \star) \to \star$ would not be possible without having a richer kind classification system. Further, promotion of type constant that themselves accept promoted types such as !Vec : * -> 'Nat -> *! are not promoted as that would involve double promotion of the type !Nat! or would require type and kind levels to be fully dependent, making the system complex. Kind polymorphic type constant are also not promoted as that would require polymorphic sorts, which is absent from the system.

\begin{itemize}
 \item \SFK squashes the types and kinds to be the same by adding the axiom $\star : \star$, or ``type is of kind type''. This necessitates a new formalism that proves that the meta-theoretic properties of \SFK are carried over from \SFC. In dependently type languages $\star : \star$ axiom amounts to adding inconsistency, however this is not a problem in Haskell as all kinds are already inhabited;
