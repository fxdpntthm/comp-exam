...
Algebraic Datatypes (ADT) can be viewed as composite datatypes of named sums of products. They enhance the expressivity of the language by allowing users to declare their own structures by possibly re-using the existing ones. As an example, a binary tree in Haskell can be declared as follows.
...
All we really want was an evaluator that evaluates the expression compositionally. In full generality, the evaluation of the expression may not result in the same type. In our case, the !IsZero! evaluates an expression to a Boolean value, while the !Plus! operator evaluates to an Integer. Thus, the type signature of the !eval! function should be agnostic of the expression result type: !eval :: FORALL a. AlgExp a -> a!. The current data constructors of !AlgExp! all have a generic return type of !AlgExp a!. The problem would disappear if we were to constraint the return type of each of the data constructors.
...
isZero (Value i)  = i == 0
isZero (Plus x y) = isZero x && isZero y
isZero (IsZero x) = undefined "impossible case"
\end{code}
\end{minipage}

The type checker has enough type information to identify that the case !IsZero x! is impossible as it has a type !GAlgExp Bool!. If we were to define the same function for !AlgExp Int!, the function would have to include it with a bogus right hand side resulting in unnecessary code clutter.

GADTs also subsume existential datatypes. Existential datatypes are useful for data abstraction, by hiding implimentation details by disallowing


Here we are able to not just constrain the structure of the data but also its return type. % In a way it is poor man's dependent types where the type is identified by the term.
