...
\begin{code}
data GAlgExpr :: * -> * where
  Value  :: FORALL a. a -> GAlgExpr a
  Plus   :: FORALL a. (a ~ Int) => GAlgExpr a
                                -> GAlgExpr a
                                -> GAlgExpr a
  IsZero :: FORALL a. (a ~ Bool) => Int -> GAlgExpr a
\end{code}
\end{minipage}%
\begin{minipage}[ht]{0.5\linewidth}
\begin{code}
...
The axom !coHtml! can be used freely to cast any term with a type !HTML! to be used in the context that expects a !String!. The elaboration scheme enables us to not only have the cake but eat it too: it has the capibility to provide type safety at zero run cost.
...
For example, in the programmer written function !mkHTML!, the the elaborated \SFC code will have a type cast instead of boxing it with a data constructor resulting in runtime efficiency and in the elaboraed !unHTML! function code would contain a type cast. If !Hmtl! were to be treated like a datatype delcaration, it would be elaborated into a !case! statement as shown above. We now formalize the above elaboration procedure using a type directed translation. We require a new category of datatypes which allows only one data constructor.
...
\section{Related Work and Some Open Problems}
