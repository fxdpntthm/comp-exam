...
\begin{abstract}
  The effects of computing technology are ubiquitous in the current modern era.
  Computers are increasingly being used to automate a wide variety of
  tasks to assist in us in all of our day
  to day activities---from transportation, communication, finance,
  nuclear power plants to, but not limited to, entertainment and
  art. To accomplish these tasks, computers run software, which in turn is written by
  humans using programming languages. The ubiquitous nature of software
  makes it mission critical; human lives and financial capital may
  depend on its correct behavior. We need to ensure that we can
  communicate with computers efficiently and
  without ambiguities. Programming languages are the interface between humans and
  computing machines. It is necessary to understand the principles of
  programming language design and its implementation.
  To this effect, this monograph is a juxtaposition of practical
  ...
  language, while identifying and studying some
  key features that gives machine verified guarantees about the
  software by making it correct by construction.
\end{abstract}
...
problems. Programmers on the other hand, turn this process on to its
...
In the programmers mind ``If my program was type
...
concrete type of !e! should be as is there no way to rule out the
...
memory. The transformations, on these structures then,
models the program logic. Algebraic datatypes (ADT) are a primary way
...
To take a step back and reflect on the situation: all we really
expect from the !eval! function is to compositionally evaluate an
expression, then why should the programmer be bothered to wrap the
...
% \TODO{Do you need a type tag here? yes}
\begin{figure}[ht]
\centering
...
until recently OCaml 4.0\cite{garrigue_gadt_2011}, Scala 3\cite{TODO},
and other languages play catch up.
